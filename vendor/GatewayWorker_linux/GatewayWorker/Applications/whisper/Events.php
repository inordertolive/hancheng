...
class Events
{
    ...
    {
        if (empty(self::$db)) {
            self::$db = new \Workerman\MySQL\Connection('127.0.0.1', '3306', 'lbphp_lwwan_com', 'JSyAHmHpmHAHEKzz', 'lbphp_lwwan_com');
        }
        ...
        switch ($message['type']) {
            ...
            case 'chatMessage':
                ...
                if(!empty($client)){
                    ...
                    self::$db->insert(self::$prefix.'operation_service_chat')->cols($serviceLog)->query();
                    unset($serviceLog);
                }
                break;
            // 转接
        ...
        foreach($kfList as $k=>$v){
            foreach($v as $key=>$vo){
                if(in_array($client_id, $vo['user_info'])){
                    ...
                    $outUser = self::$db->query("select `user_id`,`group_id` from `".self::$prefix."operation_service_log` where `client_id`= '" . $client_id . "'");
                    // 通知 客服删除退出的用户
                    if(!empty($outUser)){
                        $del_message = [
    ...
    private static function userOnlineTask($client_id, $group)
    {
        ...
        if(1 == $res['code']){
            ...
            // 检测是否开启自动应答
            $sayHello = self::$db->query('select `word`,`status` from `'.self::$prefix.'operation_service_reply` where `id` = 1');
            ...
            self::$db->insert(self::$prefix.'operation_service_log')->cols($serviceLog)->query();
            unset($serviceLog);
