	...
	browserMD5File(items, function (err, md5) {
		...
		$.post('/api/v1/5d01f888aef57',{
			...
			if(result.code == '304'){				
				...
				var html = '<video src="'+ response.path +'" controls="controls" style="width:200px">您的浏览器不支持 video 标签。</video><p>' + response.name + '</p> ';
				par.find('[data-upvideo-show]').html(html);		
				par.find('[data-upvideo-name]').html('上传成功');					
				return false;
			}
			if(result.code != '200'){
				par.find('[data-upvideo-name]').html(result.info)
				return false;
			}
			...
			var saveToken  = response.saveToken;
			...
			$.ajax({
				...
				success: function(data) {
					$.post('/api/v1/5d0202ab86d31', { saveToken : saveToken },function(res){
						if(res.code >0){
							par.find('[data-upvideo-val]').val(res.data.id);
							var html = '<video src="'+ res.data.path +'" controls="controls" style="width:200px">您的浏览器不支持 video 标签。</video><p>' + res.data.name + '</p> ';
							par.find('[data-upvideo-show]').html(html);		
							par.find('[data-upvideo-name]').html('上传成功');	
							return ;
						}
