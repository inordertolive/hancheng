        ...
        if (!$result) {
            ...
            foreach ($modules as &$module) {
                ...
                if (isset($module['system_module']) && $module['system_module'] == '1') {
                    $module['actions'] = '<button class="btn btn-xs btn-noborder btn-default btn-flat" type="button" disabled>不可操作</button>';
                ...
                switch ($module['status']) {
                    ...
                    case '-1': // 未安装
                        ...
                        $module['actions'] = '<a class="btn btn-xs btn-noborder btn-default btn-flat" href="'.url('install', ['name' => $module['name']]).'">安装</a>';
                        $module['status_class'] = 'text-info';
                        $module['status_info'] = '<i class="fa fa-fw fa-th-large"></i> 未安装';
                        break;
                    case '0': // 禁用
                        $module['bg_color'] = 'warning';
                        $module['actions'] = '<a class="btn btn-xs btn-noborder btn-default btn-flat ajax-get confirm" href="'.url('enable', ['ids' => $module['id']]).'">启用</a> ';
                        $module['actions'] .= '<a class="btn btn-xs btn-noborder btn-default btn-flat" href="'.url('uninstall', ['name' => $module['name']]).'">卸载</a> ';
                        $module['status_class'] = 'text-warning';
                        $module['status_info'] = '<i class="fa fa-ban"></i> 已禁用';
                        break;
                    case '1': // 启用
                        $module['bg_color'] = 'success';
                        $module['actions'] = '<a class="btn btn-xs btn-noborder btn-default btn-flat ajax-get confirm" href="'.url('update', ['name' => $module['name']]).'">更新</a> ';
                        $module['actions'] .= '<a class="btn btn-xs btn-noborder btn-default btn-flat ajax-get confirm" href="'.url('disable', ['ids' => $module['id']]).'">禁用</a> ';
                        $module['actions'] .= '<a class="btn btn-xs btn-noborder btn-default btn-flat" href="'.url('export', ['name' => $module['name']]).'">导出</a> ';
                        $module['actions'] .= '<a class="btn btn-xs btn-noborder btn-default btn-flat" href="'.url('uninstall', ['name' => $module['name']]).'">卸载</a> ';
                        $module['status_class'] = 'text-success';
                        $module['status_info'] = '<i class="fa fa-check"></i> 已启用';
                        break;
