            ...
            foreach ($addons as &$plugin) {
                switch ($plugin['status']) {
                    ...
                    case '-3': // 插件信息缺失
                        ...
                        $plugin['status_class'] = 'text-danger';
                        $plugin['status_info'] = '<i class="fa fa-times"></i> 已损坏';
                        $plugin['actions'] = '<button class="btn btn-xs btn-noborder btn-default btn-flat" type="button" disabled>不可操作</button>';
                        break;
                    case '-2': // 入口文件不存在
                    ...
                    case '-1': // 未安装
                        ...
                        $plugin['actions'] = '<a class="btn btn-xs btn-noborder btn-default ajax-get confirm btn-flat" href="'.url('install', ['name' => $plugin['name']]).'">安装</a>';
                        $plugin['status_class'] = 'text-info';
                        $plugin['status_info'] = '<i class="fa fa-fw fa-th-large"></i> 未安装';
                        break;
                    case '0': // 禁用
                        $plugin['bg_color'] = 'warning';
                        $plugin['actions'] = '<a class="btn btn-xs btn-noborder btn-default ajax-get confirm btn-flat" href="'.url('enable', ['ids' => $plugin['id']]).'"><i class="fa fa-check-circle pr5"></i> 启用</a> ';
                        $plugin['actions'] .= '<a class="btn btn-xs btn-noborder btn-default ajax-get confirm btn-flat" data-tips="如果包括数据库，将同时删除数据库！" href="'.url('uninstall', ['name' => $plugin['name']]).'"><i class="fa fa-recycle pr5"></i> 卸载</a> ';
                        if (isset($plugin['config']) && $plugin['config'] != '') {
                            $plugin['actions'] .= '<a class="btn btn-xs btn-noborder btn-default btn-flat" href="'.url('config', ['name' => $plugin['name']]).'"><i class="fa fa-cog pr5"></i> 设置</a> ';
                        }
                        if ($plugin['admin'] != '0') {
                            $plugin['actions'] .= '<a class="btn btn-xs btn-noborder btn-default  btn-flat" href="'.url('manage', ['name' => $plugin['name']]).'"><i class="fa fa-list pr5"></i> 管理</a> ';
                        }
                    ...
                    case '1': // 启用
                        ...
                        $plugin['actions'] = '<a class="btn btn-xs btn-noborder btn-default ajax-get confirm btn-flat" href="'.url('disable', ['ids' => $plugin['id']]).'"><i class="fa fa-ban pr5"></i> 禁用</a> ';
                        $plugin['actions'] .= '<a class="btn btn-xs btn-noborder btn-default ajax-get confirm btn-flat" data-tips="如果包括数据库，将同时删除数据库！" href="'.url('uninstall', ['name' => $plugin['name']]).'"><i class="fa fa-recycle pr5"></i> 卸载</a> ';
                        if (isset($plugin['config']) && $plugin['config'] != '') {
                            $plugin['actions'] .= '<a class="btn btn-xs btn-noborder btn-default btn-flat" href="'.url('config', ['name' => $plugin['name']]).'"><i class="fa fa-cog pr5"></i> 设置</a> ';
                        }
